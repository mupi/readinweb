...
public class ReadInWebAdminLogicImpl implements ReadInWebAdminLogic {
	...
	private static final Logger log = Logger
			.getLogger(ReadInWebCourseLogic.class);
	...
	public ReadInWebCourseData getReadInWebData(Course c) {
		ReadInWebCourseData riwData = new ReadInWebCourseData(sakaiProxy
				.getReadInWebSites(c.getId()).size(), 0, 0, 0);
		...
		for (Site s : sites) {
			...
			for (User u : users) {
				...
				riwData.sumCountGraduates(common.isUserBLocked(common
						.getUserControl(u.getId(), s.getId())));
			}
		}
		return riwData;
	}
