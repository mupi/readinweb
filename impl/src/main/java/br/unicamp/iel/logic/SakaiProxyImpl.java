...
public class SakaiProxyImpl implements SakaiProxy {
	...
	public Long getCourseId() {
		try {
			Site site = siteService.getSite(toolManager.getCurrentPlacement()
					.getContext());
			return site.getProperties().getLongProperty(
					Property.COURSE.getName());
	...
	public Long getManagerCourseId() {
		try {
			...
			if (ts != null) {
				ToolConfiguration tc = siteService
						.findTool(ts.getPlacementId());
				if (tc != null) {
					SitePage sp = tc.getContainingPage();
					ResourceProperties p = sp.getProperties();
					String course = p.getProperty(Property.COURSEMANAGED
							.getName());
		...
		try {
			...
			for (SitePage sp : pages) {
				...
				for (ToolConfiguration tc : tools) {
					...
					String courseManaged = (String) p
							.get(Property.COURSEMANAGED.getName());
	...
	public Site createSite(String siteId) {
		...
		if (this.siteService.allowAddSite(siteId)) {
			try {
				...
				AuthzGroup ag = authzGroupService.getAuthzGroup(site
						.getReference());
	...
	public Site getCurrentSite() {
		try {
			return siteService.getSite(toolManager.getCurrentPlacement()
					.getContext());
		} catch (IdUnusedException e) {
			e.printStackTrace();
		}
	...
	public SitePage findCurrentPage() {
		...
		if (ts != null) {
			...
			if (tool != null) {
				...
				try {
					Site s = siteService.getSite(toolManager
	...
	public Site getLastAddedSiteByProperty(Long course) {
		...
		return new ArrayList<Site>(siteService.getSites(SelectionType.ANY,
				null, null, m, SortType.CREATED_BY_DESC, null)).get(0);
	}
	...
	public Site getLastModifiedSiteByProperty(Long course) {
		...
		return new ArrayList<Site>(siteService.getSites(SelectionType.ANY,
				null, null, m, SortType.MODIFIED_BY_DESC, null)).get(0);
	}
	...
	public Session adminSessionStart() {
		...
		org.sakaiproject.event.api.UsageSession session = usageSessionService
