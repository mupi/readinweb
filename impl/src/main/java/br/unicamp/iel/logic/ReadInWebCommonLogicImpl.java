...
public class ReadInWebCommonLogicImpl implements ReadInWebCommonLogic {
	...
	private static final Logger log = Logger
			.getLogger(ReadInWebCommonLogicImpl.class);
	...
	public ReadInWebUserControl getUserControl(String userId, String siteId) {
		ReadInWebUserControl userControl = dao.findOneBySearch(
				ReadInWebUserControl.class, new Search(new Restriction[] {
						new Restriction("user", userId),
						new Restriction("site", siteId), }));

		if (userControl == null) {
			userControl = new ReadInWebUserControl(
					BlockStateTypes.UNBLOCKED.getValue(), null, null, userId,
					siteId, 0);
	...
	public List<Justification> getUserJustifications(String user, String siteId) {
		Search search = new Search(
				new Restriction[] { new Restriction("user", user),
						new Restriction("site", siteId), });
	...
	public List<Justification> getSiteJustifications(String siteId) {
		Search search = new Search(new Restriction[] { new Restriction("site",
				siteId), });
	...
	public String getDefaultCoursePropertyString(Course course) {
		...
		for (Module m : modules) {
			...
			for (Activity a : activities) {
				...
				j_m.get("activities").asObject().add(a.getId().toString(), j_a);
			}
		}
		return jo.toString();
	}
	...
	public String getCoursePropertyString(Site site) {
		return sakaiProxy
				.getStringProperty(site, Property.COURSEDATA.getName());
	}
	...
	public void setCoursePropertyString(String siteId, String value) {
		...
		sakaiProxy
				.setStringProperty(site, Property.COURSEDATA.getName(), value);
	}
	...
	public Long[] getAllPublishedActivities(String siteId) {
		CourseProperties courseProperties = new CourseProperties(
				JsonObject.readFrom(getCoursePropertyString(sakaiProxy
						.getSite(siteId))));
	...
	public boolean idiomCourseExists(String idiom) {
		return (dao.findBySearch(Course.class, new Search("idiom", idiom)))
				.size() > 0;
	}
	...
	public Long countPublishedActivities(Site site) {
		CourseProperties courseProperties = new CourseProperties(
				JsonObject.readFrom(getCoursePropertyString(site)));
