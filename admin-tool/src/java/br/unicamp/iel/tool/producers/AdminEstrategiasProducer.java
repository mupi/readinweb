 ...
 *
 */
public class AdminEstrategiasProducer implements ViewComponentProducer,
		ViewParamsReporter, NavigationCaseReporter {

	private static Log logger = LogFactory
			.getLog(AdminEstrategiasProducer.class);
	...
	public void fillComponents(UIContainer tofill, ViewParameters viewparams,
		...
		for (Strategy strategy : strategies) {
			if (strategy.getType().equals(StrategyType.LANGUAGE.getValue())) { // aquisicao
				UIBranchContainer rowMod = UIBranchContainer.make(tofill,
						"li-rowsEstLi:");

				UIForm langForm = UIForm.make(rowMod, "language_strategy_form");
				langForm.parameters.add(new UIELBinding(
						"#{AdminActivityBean.strategyId}", Long
								.toString(strategy.getId())));

				UIInput.make(langForm, "language_strategy_data",
						"#{AdminActivityBean.strategyData}", strategy.getBody());
			...
			} else if (strategy.getType().equals(
					StrategyType.READING.getValue())) { // leitura
				UIBranchContainer rowMod = UIBranchContainer.make(tofill,
						"li-rowsEstLe:");

				UIForm readForm = UIForm.make(rowMod, "reading_strategy_form");
				readForm.parameters.add(new UIELBinding(
						"#{AdminActivityBean.strategyId}", Long
								.toString(strategy.getId())));
		...
		addStrategyForm.parameters.add(new UIELBinding(
				"#{AdminActivityBean.strategyActivity}", Long.toString(activity
						.getId())));
	...
	public List<NavigationCase> reportNavigationCases() {
		...
		l.add(new NavigationCase(CourseComponents.SAVED,
				new CourseViewParameters(AdminEstrategiasProducer.VIEW_ID,
						true, false, false)));
