 ...
 *
 */
public class JustificationManagerProducer implements ViewComponentProducer,
		ViewParamsReporter {

	public static final String VIEW_ID = "justificativa-single";

	private static Log logger = LogFactory
			.getLog(JustificationManagerProducer.class);
	...
	public void fillComponents(UIContainer tofill, ViewParameters viewparams,
		...
		JustificationViewParameters parameters = (JustificationViewParameters) viewparams;

		Justification justification = logic
				.getJustification(parameters.justification);
		...
		if (!JustificationStateTypes.isOld(justification.getState())) {
			...
			UIForm evaluateForm = UIForm.make(container,
					"current_evaluate_form");
			evaluateForm.parameters.add(new UIELBinding(
					"#{JustificationBean.justificationId}", justification
							.getId()));
			// current_refuse
			...
			UIOutput.make(container, "sent_date", justification.getSentDate()
					.toString());

			UIOutput evaluatedDate = UIOutput.make(container, "evaluated_date");

			evaluatedDate
					.setValue(justification.getEvaluatedDate() == null ? "Ainda não avaliado"
							: justification.getEvaluatedDate().toString());

			List<JustificationMessage> messages = logic
					.getJustificationMessages(justification);

			for (JustificationMessage jm : messages) {
				UIBranchContainer currentMessage = UIBranchContainer.make(
						tofill, "current_justification_message:");
				...
				if (student.getId().equals(jm.getUser())) {
					...
					currentMessage.decorate(new UIStyleDecorator(
							"alert-warning"));
				}
				UIOutput.make(currentMessage, "current_justification_body",
						jm.getMessage());
				UIOutput.make(currentMessage, "current_justification_date", jm
						.getDateSent().toString());
			}

			UIBranchContainer messageContainer = UIBranchContainer.make(
					container, "send_message_item:");
			UIForm messageForm = UIForm.make(messageContainer, "message_form");

			messageForm.parameters.add(new UIELBinding(
					"#{JustificationBean.justificationId}", justification
							.getId()));

			UIOutput.make(messageForm, "message_user", logic.getUser()
					.getFirstName());

			UIInput.make(messageForm, "message", "#{JustificationBean.message}");
		...
		UIOutput.make(tofill, "activities_completed",
		...
		for (UserAccess ua : access) {
			UIBranchContainer row = UIBranchContainer.make(tofill,
					"activity_access_detail:");
			...
			UIOutput.make(row, "activity_total_accesses", ua.getTotal()
					.toString());
		}

		List<Justification> justifications = logic
				.getUserJustifications(justification.getUser());

		for (Justification j : justifications) {
			if (!JustificationStateTypes.toShow(j.getState())) {
				UIBranchContainer old_justifications = UIBranchContainer.make(
						tofill, "old_justifications:");
				if (j.getState() >= 4) {
					old_justifications.decorate(new UIStyleDecorator(
							"alert-danger"));
				}
				...
				List<JustificationMessage> jmessages = logic
						.getJustificationMessages(j);
				System.out.println("Num mens: " + jmessages.size());
				for (JustificationMessage jm : jmessages) {
					UIBranchContainer oldMessage = UIBranchContainer.make(
							old_justifications, "old_justification_message:");
					UIOutput messageUser = UIOutput.make(oldMessage,
							"old_justification_sender", "Professor");
					if (student.getId().equals(jm.getUser())) {
						messageUser.setValue(student.getDisplayName());
					} else {
						oldMessage.decorate(new UIStyleDecorator(
								"alert-warning"));
					}
					UIOutput.make(oldMessage, "old_justification_body",
							jm.getMessage());
					UIOutput.make(oldMessage, "old_justification_date", jm
							.getDateSent().toString());
				}
			}
		}
