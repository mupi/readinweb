...
public class TextProducer implements ViewComponentProducer, ViewParamsReporter {
	...
	public void fillComponents(UIContainer tofill, ViewParameters viewparams,
		...
		if ((activity.getPrereading() != null)
			...
			UIVerbatim
					.make(tofill, "prelecture_text", activity.getPrereading());
		}
		...
		String audioString = ""
				...
				// module.getPosition() + "a" + activity.getPosition() +
				// ".mp3' controls>"
				+ "    src='/readinweb-uploads/english/audio/"
				// + "m"
				// + module.getPosition() + "a" + activity.getPosition()
				// + ".mp3' "
				+ activity.getAudiofile() + "' controls>"
		...
		List<Question> l_q = new ArrayList<Question>(
				logic.getQuestions(activity));
		for (Question q : l_q) {
			...
			answer_form.viewparams = new SimpleViewParameters(
					UVBProducer.VIEW_ID);
